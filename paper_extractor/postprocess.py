...
def build_fallback_parse_prompt(stage: str, raw_text: str, paper_id: str, extra_context: Dict[str, Any] | None = None) -> str:
    ...
    return (
        ...
        "1. 只输出 JSON，不要解释。\n"
        "2. 不要编造原始输出中没有的信息。\n"
        "3. 若 stage=text_post_parse，则输出一个 JSON object。\n"
        "4. 若 stage=multimodal_post_parse，则输出一个 Figure 级 JSON object，字段应尽量包含 "
        "paper_id, figure_id, image_type, description, confidence。\n\n"
        f"paper_id: {paper_id}\n"
        f"stage: {stage}\n"
