...
【总体抽取目标 — 信息密度优先】
4. 目标不是生成简短摘要，而是尽可能保留**高信息密度、高证据价值**的结构化信息。优先抽取：
...
26. 输出前自检：JSON 可被标准解析器解析；无尾逗号、无注释、双引号键名；全树不存在 number/boolean 类型。附带模板中字段含义以**键名与嵌套路径**为准，并与 `paper_entity_schema.jsonc` 内中文注释一致（程序传参前已去注释）。
```
