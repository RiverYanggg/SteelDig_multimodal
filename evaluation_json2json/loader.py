...
def load_paper_pairs(truth_dir: str | Path, prediction_root: str | Path) -> list[PaperPair]:
    ...
    for paper_key, truth_path in _iter_truth_paths(truth_root):
        prediction_path = _prediction_path(pred_root, paper_key)
...
def _iter_truth_paths(truth_root: Path):
    nested_paths = sorted(truth_root.glob("*/final/text_extraction.json"))
    if nested_paths:
        for truth_path in nested_paths:
            yield truth_path.parents[1].name, truth_path
        return

    flat_paths = sorted(truth_root.glob("*.json"))
    for truth_path in flat_paths:
        yield truth_path.stem, truth_path


def _prediction_path(pred_root: Path, paper_key: str) -> Path:
    flat_path = pred_root / f"{paper_key}.json"
    if flat_path.exists():
        return flat_path
    return pred_root / paper_key / "final" / "text_extraction.json"
